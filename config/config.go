...
func (c *Config) HandleSetup() error {
	if _, err := c.ini.GetSection(INI_SETUP); err == nil {
		...
		if err != nil || o == nil {
			// initConfig() always creates row id=1 at startup, so this is
			// "should never happen". Bail loudly instead of dereferencing nil
			// when assigning RedirectUrl / SecretPath below.
			log.Error("config: can't get config from db")
			return fmt.Errorf("config: setup aborted, db config row missing")
		}
