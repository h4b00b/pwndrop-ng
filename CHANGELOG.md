...
- [x] Admin QoL: expiry quick-presets in the Edit modal (`+1h / +1d / +7d / +30d / Never`) under the `datetime-local` picker. Buttons compute the absolute timestamp client-side via the existing `tsToLocalInput` helper and write to the same `expire_local` model the picker uses, so the existing PUT path is unchanged.
- [x] Version constant bumped to `2.0.9`; frontend `package.json` synced.
