...
func main() {
	...
	log.Info("%s version: %s", config.ProductName, config.Version)
	...
	core.StartCleanupLoop()
	api.SetCleanupRunner(core.RunCleanupNow)
	api.SetForgetFileLock(core.ForgetFileLock)
	select {
	case _ = <-ch_exit:
		log.Fatal("aborting")
		os.Exit(1)
