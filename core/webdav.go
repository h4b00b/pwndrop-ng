...
func (fs *WebDavFS) OpenFile(ctx context.Context, name string, flag int, perm os.FileMode) (webdav.File, error) {
	...
	// Refuse anything that isn't strictly read-only. The golang.org/x/net/webdav
	// PUT path calls OpenFile with O_WRONLY|O_CREATE|O_TRUNC and trusts the
	// filesystem to apply it — passing that through to os.OpenFile would
	// truncate the served blob to 0 bytes BEFORE our Write() guard ever runs.
	// Even though Write() returns "not supported", the damage is done at open
	// time. Treat the FS as read-only at the open boundary.
	if flag&(os.O_WRONLY|os.O_RDWR|os.O_TRUNC|os.O_APPEND|os.O_CREATE) != 0 {
		return nil, fmt.Errorf("openfile: read-only")
	}
